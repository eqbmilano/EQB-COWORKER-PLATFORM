...
export const Card: React.FC<CardProps> = ({ children, className = "", variant = "glass" }) => {
  const variants = {
    glass:
      "rounded-lg border border-white/20 bg-white/10 backdrop-blur-md shadow-lg hover:bg-white/15 transition-colors duration-200",
  ...
  return <div className={`p-6 ${variants[variant]} ${className}`}>{children}</div>;
};
